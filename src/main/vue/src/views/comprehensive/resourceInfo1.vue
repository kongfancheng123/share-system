  ...
  <div class="pad20">
      ...
      <el-table-column     label="操作">
        <template slot-scope="scope">
          ...
          <el-button size="mini"  v-if='(scope.row.state==="已租赁"||scope.row.state==="已超时")&&(user.leaguerName===scope.row.userName)'
                     @click="backResource(scope.$index, scope.row,'更新',0)">归还</el-button>

          <el-button size="mini"   v-if='scope.row.state==="可预约"'
                     @click="appointResource(scope.$index, scope.row,'更新',0)">预约</el-button>

          <el-button size="mini"  v-if='scope.row.state==="已预约"&&user.leaguerName===scope.row.userName'
                               @click="warnEvent(scope.$index, scope.row,'租赁',0)">租赁</el-button>
    ...
    <el-dialog :title="title"
      ...
      <el-form :model="formWarn"
        ...
        <el-form-item label="资源名称"
                      prop="resourceName">
          <el-input v-model.number="formWarn.resourceName" disabled
        ...
        <el-form-item label="资源描述"
                      prop="resourceDesc">
          <el-input v-model.number="formWarn.resourceDesc" disabled
        ...
        <el-form-item label="归还时间"
                                      prop="backTime">
                          <el-date-picker v-model="formWarn.backTime"
                                          type="datetime"
                                          placeholder="选择归还时间">
                          </el-date-picker>
                        </el-form-item>
  ...
  methods: {
    ...
    */
    httpRequst(urlName, formName, dialogFlag, sucessMsg, failMsg) {
      let vm = this
      var str =vm[formName].backTime+"";
      console.log(vm[formName].backTime);
      if(str.indexOf("中国标准时间")>-1){
        vm[formName].backTime = vm.formatTime(+vm[formName].backTime)
      }
      console.log(vm[formName].backTime);
  ...
  */
   ...
   */
    warnEvent(index, row, txt, num) {
      ...
      vm.formWarn.backTime = row.backTime
    ...
    warnSumit(formName) {
      ...
      vm.$refs[formName].validate(valid => {
        if (valid) {
          vm.httpRequst('leaseResource',formName,'warnFlag','更新成功','更新失败')
        } else {
          return false
        }
      })
    ...
    /*预约资源 */
    appointResource(index, row, txt, num) {
          let vm = this
    ...
    /*归还资源 */
    backResource(index, row, txt, num) {
          let vm = this
